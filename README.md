...
### 输出数据
输出数据至少几百`M`，较大，不适合使用默认的 Git 来存储。请不要把输出数据加入 `commit`。
...
读取 `data/sources.csv, data/bandwidth.csv, data/sample.csv` 文件中的配置，生成从 -0.5s 开始，给定长度 T 和采样率 F (`data/sample`)下台站1(i = 1)分别接收各个源 $j$ 的信号 $A_{ij}$。

例如：sample.csv中定义采样时间 T 为3s时，你生成的数据对应的时间应为 $[-0.5, 2.5)s$。
...
| TIME       | A11         | A21         | ... | A_N1         |
| ---------- | ----------- | ----------- | --- | ----------- |
| -0.500     | A11[0]      | A21[0]      | ... | A_N1[0]      |
| -0.5+1/F   | A11[1]      | A21[1]      | ... | A_N1[1]      |
| ...        | ...         | ...         | ... | ...         |
| T-0.5-1/F  | A11[TF - 1] | A21[TF - 1] | ... | A_N1[TF - 1] |

除去描述性的第一行和第一列后，它们被保存在 dataset `WAVES` 里，是一个 `(TF, N)` 的 float 数组，其中 N 是源的个数。

其物理意义是从 -0.5s 开始，各个源开始辐射电磁波，并且可以被1号台站接收(其余台站可能提前或延后接收到)；到 T-0.5s，各个源停止辐射，1号台站不能接收到信号(其余台站可能提前或延后停止)。
...
读取 `waves.h5` 和 `data/stations.csv, data/speed_of_light.csv, data/sample.csv`，利用 `wave.h5`，考虑各基站位置不同造成的时间偏移，进一步生成 $A_{ij} (i>1)$, 并叠加 $\sum_i A_{ij}$得到各个台站应当接收到的信号$S_i$。

输出 `ideal_signal.h5` 文件，文件的内容是各个台站在 [-0.5, T-0.5)s 中接收到的总信号。

| TIME       | S1          | S2          | ... | S_M         |
| ---------- | ----------- | ----------- | --- | ----------- |
| -0.500     | S1[0]       | S2[0]       | ... | S_M[0]      |
| -0.5+1/F   | S1[1]       | S2[1]       | ... | S_M[1]      |
| ...        | ...         | ...         | ... | ...         |
| T-0.5-1/F  | S1[TF - 1]  | S2[TF - 1]  | ... | S_M[TF - 1] |

它们被保存在 dataset `IDEAL_SIGNALS` 里，去除掉第一行和第一列，为一个 `(TF, M)` 的 float 数组。

#### `plot_wave.py`

读取 `waves.h5`，`ideal_signal.h5` 和 `data/sample.csv`, 画出信号的时域波形。其中包括：

1. 1号源在1号台站处的 [0, 10)$\mu s$ 的时域波形;
2. 2号源在1号台站处的 [0, 10)$\mu s$ 的时域波形;
3. 3号源在1号台站处的 [0, 10)$\mu s$ 的时域波形;
4. 1号台站的[0, 10)$\mu s$ 的时域信号。

它们应当按照顺序（左上->右上->左下->右下）地布置在一张图片中，并有必要的标题和标注，其中横坐标单位为 $\mu s$。

####  `plot_wave_spectrum.py`

读取 `waves.h5` `ideal_signal.h5` 和 `data/sample.csv`, 画出信号的频域波形，只需要幅度谱。图中需要包括：

1. 1号源在1号台站处的 [0, 10)$\mu s$ 的频域幅度谱;
2. 2号源在1号台站处的 [0, 10)$\mu s$ 的频域幅度谱;
3. 3号源在1号台站处的 [0, 10)$\mu s$ 的频域幅度谱;
4. 1号台站的 [0, 10)$\mu s$ 的频域幅度谱。
