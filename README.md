...
读取 `data/sources.csv, data/bandwidth.csv, data/sample.csv` 文件中的配置，生成从 -0.5s 开始，给定长度 $T$ 和采样率 $F$ (`data/sample.csv`)下台站1（i = 1）分别接收的各个源 $j$ 的信号 $A_{ij}$。
...
读取 `data/ostrength.csv` 和 `data/stations.csv` 来计算台站时间相对于自身理想状态的偏移的随机数。最终得到各个台站各自的随机时间偏移，保存在 `offset.csv` 中，共 $N$ 行，格式如下（注意实际数据需要更多的有效数字）：
...
读取 `waves.h5` 和 `data/stations.csv, data/speed_of_light.csv, data/sample.csv`，利用 `wave.h5`，考虑各基站位置不同造成的时间偏移，进一步生成 $A_{ij} (i>1)$，并叠加得到各个台站应当接收到的信号 $S_i=\sum_j A_{ij}$。
...
它们被保存在 dataset `IdealSignals` 里，同样去除掉第一行和第一列，为一个 `(TF, M)` 的 float 数组。
