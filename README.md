...
| ...        | ...         | ...         | ...         |
| 2.49999995 | A11[6e7 - 1]| A21[6e7 - 1]| A31[6e7 - 1]|

``A11[S]`是长度为 `S` 的一维数组，其中 `S` 是信号的总长，它是采样率和采样时间的乘积 (`data/sample.csv`)。

除去描述性的第一行和第一列后，它们被保存在 dataset `WAVES` 里，是一个 `(S, 3)` 的 float 数组。
...
| ...         | ...         | ...         | ...         | ...         | ...         |
| 2.49999995  | S1[S - 1]   | S2[S - 1]   | S3[S - 1]   | S4[S - 1]   | S5[S - 1]   |

它们被保存在 dataset `IDEAL_SIGNALS` 里，去除掉第一行和第一列，为一个 `(S, 5)` 的 float 数组。
...
最终需要得到各个台站接收的信号中 [0, 2)s 的部分，作为(X, 5)的 float 数组，保存在`signal.h5`文件的 dataset `SIGNALS`中。X 是 [0, 2)s 的采样个数，
